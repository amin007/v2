...
function parseMsic(string $raw): array
{
	...
	// ── Peta nama penuh setiap seksyen ───────────────────────────
	$seksyenNames = [
		'A' => 'PERTANIAN, PERHUTANAN DAN PERIKANAN',
		'B' => 'PERLOMBONGAN DAN PENGKUARIAN',
		'C' => 'PEMBUATAN',
		'D' => 'BEKALAN ELEKTRIK, GAS, WAP DAN PENDINGIN UDARA',
		...
		'H' => 'PENGANGKUTAN DAN PENYIMPANAN',
		'I' => 'PENGINAPAN DAN AKTIVITI PERKHIDMATAN MAKANAN',
		'J' => 'AKTIVITI PENERBITAN, PENYIARAN DAN PENGHASILAN SERTA PENGEDARAN KANDUNGAN',
		'K' => 'TELEKOMUNIKASI, PENGATURCARAAN KOMPUTER, PERUNDINGAN, INFRASTRUKTUR PENGKOMPUTERAN DAN AKTIVITI PERKHIDMATAN MAKLUMAT LAIN',
		'L' => 'AKTIVITI KEWANGAN DAN INSURANS/TAKAFUL',
		'M' => 'AKTIVITI HARTANAH',
		'N' => 'AKTIVITI PROFESIONAL, SAINTIFIK DAN TEKNIKAL',
		'O' => 'AKTIVITI PENTADBIRAN DAN KHIDMAT SOKONGAN',
		'P' => 'PENTADBIRAN AWAM DAN PERTAHANAN: KESELAMATAN SOSIAL WAJIB',
		'Q' => 'PENDIDIKAN',
		'R' => 'AKTIVITI KESIHATAN KEMANUSIAAN DAN KERJA SOSIAL',
		'S' => 'SENI, SUKAN DAN REKREASI',
		'T' => 'AKTIVITI PERKHIDMATAN LAIN',
		'U' => 'AKTIVITI ISI RUMAH SEBAGAI MAJIKAN; AKTIVITI MENGELUARKAN BARANGAN DAN PERKHIDMATAN YANG TIDAK DAPAT DIBEZAKAN OLEH ISI RUMAH UNTUK KEGUNAAN SENDIRI',
		'V' => 'AKTIVITI BADAN DAN PERTUBUHAN LUAR WILAYAH',
	];

	// Bendera: seksyen baharu dikesan tetapi baris SEKSYEN belum dioutput.
	// Akan dioutput apabila BAHAGIAN pertama dijumpai (untuk dapatkan kodnya).
	$pendingSeksyen = null;
	...
	foreach ($lines as $line) {
		...
		// Kemas kini huruf seksyen aktif dan tandakan sebagai pending
		// (baris SEKSYEN akan dioutput apabila BAHAGIAN pertama dijumpai)
		if (preg_match('/^MSIC\s+\d+\s+SEKSYEN\s+([A-Z])/i', $trimmed, $m)) {
			$seksyen        = strtoupper($m[1]);
			$pendingSeksyen = $seksyen;
			continue;
		}
		...
		// Jika ada seksyen baharu yang belum dioutput ($pendingSeksyen),
		// output baris SEKSYEN dahulu menggunakan kod BAHAGIAN pertama ini.
		// Kemudian output baris BAHAGIAN seperti biasa.
		if (preg_match('/^BAHAGIAN\s+(\d+)\s*[:\-]?\s*(.*)/i', $trimmed, $m)) {
			...
			// Output baris SEKSYEN jika belum dioutput
			if ($pendingSeksyen !== null) {
				$namaSeksyen = $seksyenNames[$pendingSeksyen] ?? $pendingSeksyen;
				$tajukSeksyen = 'SEKSYEN ' . $pendingSeksyen . ' : ' . $namaSeksyen;
				$rows[]       = makeCsvRow($pendingSeksyen, $kodBahagian, $tajukSeksyen, '_', '_');
				$pendingSeksyen = null;
			}
		...
		<table>
			...
			<tbody>
				...
				foreach ($csvRows as $row):
					...
					$rowClass = '';
					if (str_starts_with($col4, 'SEKSYEN')) {
						$rowClass = 'row-seksyen';
					} elseif (str_starts_with($col4, 'BAHAGIAN')) {
