...
###################################################################################################
#--------------------------------------------------------------------------------------------------
if ( ! function_exists('binaJadual03')):
	function binaJadual03($row,$jadual)
	{
		$output = null;
		$bilBaris = count($row);
		$cetak_tajuk_utama = false;# mula bina jadual
		...
		#---------------------------------------------------------------------
			# papar baris data dari tatasusunan
			$output .= "\n\t<tr>";
			$kunci_semua = array_keys($row[$kira]);
			$lompat = []; # simpan indeks yang perlu dilangkau
			foreach ( $row[$kira] as $key=>$data ) :
				$idx = array_search($key, $kunci_semua);
				# langkau medan yang telah digabungkan
				if ( in_array($idx, $lompat) ) :
					$output .= "<!-- $key|$kira -->";
					continue;
				endif;
...
#--------------------------------------------------------------------------------------------------
if ( ! function_exists('dariCsvPanjangKeJson')):
	function dariCsvPanjangKeJson($namaFail,$panjangTeks = 1024)
	{
		# open csv file
		...
		# read csv headers
		$key = fgetcsv($fp,$panjangTeks,",");
		# parse csv rows into array
		$json = array();
		while ($row = fgetcsv($fp,$panjangTeks,","))
		{
...
if ( ! function_exists('panggilDataTable01')):
	function panggilDataTable01($tajuk,$data,$pilih,$cariData = null)
	{
		//define ('URL', dirname('http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF']));
		define ('URL', $_SERVER['SCRIPT_NAME']);
		list($urlcss,$urljs) = linkCssJs();//linkLocalCssJs();
...
if ( ! function_exists('panggilDataTable02')):
	function panggilDataTable02($tajuk,$data,$pilih,$cariData = null)
	{
		//define ('URL', dirname('http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF']));
		define ('URL', $_SERVER['SCRIPT_NAME']);
		list($urlcss,$urljs) = linkCssJs();//linkLocalCssJs();
...
if ( ! function_exists('panggilDataTable03')):
	function panggilDataTable03($tajuk,$data,$pilih,$cariData = null)
	{
		//define ('URL', dirname('http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF']));
		define ('URL', $_SERVER['SCRIPT_NAME']);
		list($urlcss,$urljs) = linkCssJs();//linkLocalCssJs();
...
#--------------------------------------------------------------------------------------------------
if ( ! function_exists('panggilDataTable06')):
	function panggilDataTable06($tajuk,$data,$pilih)
	{
		//define ('URL', dirname('http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF']));
